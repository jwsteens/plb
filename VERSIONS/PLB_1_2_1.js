...
bot.on('ready', () => {
    ...
    bot.user.setActivity("over the logs", { type: "WATCHING" })
    .then()
    .catch(log("Could not set status.", "Error", "System"));
})
...
bot.on('message', (message) => {
    ...
    switch(args[0]) {
        case 'logpatrol':
            ...
            if(args[4]) {
                message.channel.send(`Make sure to use the correct format: \`${prefix}logpatrol [usernanme] (hours) [minutes]\``);
                return;
            }
            if(!args[3]) {
                if(!args[1] || !args[2]) {
        ...
        case 'alllogs':
            ...
            while (true) {
                dirent = interactivedatadir.readSync();
                if (dirent == null) {
                    break;
                } else {
